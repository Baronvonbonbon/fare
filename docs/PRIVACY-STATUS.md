...
| Stablecoin earnings had no private exit at all | **Closed.** The note path was native-only: `depositShieldNoteZK` called `depositNative`, and the buffer and the bucket ladder were single native values. A driver or venue settled in USDC could only `withdrawToken` to a named address. `insertShieldNoteToken` → `depositShieldNoteTokenZK` gives the token payout the same treatment, ending in a `depositAsset(1337, …)` to the pool. The asset is bound by giving each asset **its own note tree**: the circuit's public signals are `[root, nullifierHash, bucket, ksCommitment]` with no asset among them, so the root is what carries it — a USDC-tree proof can never satisfy the native root window. No circuit change, and therefore no second trusted setup. Asserted in `test/shieldnote-token.test.ts`. |
...
| **Anonymity is only as large as usage** | An empty note tree is an anonymity set of one. The mechanisms are right; the privacy is whatever adoption provides. **Per-asset trees make this sharper, not softer:** native and USDC notes are now separate crowds, and the shielded pool's own escrow was already per-asset, so a USDC note hides among USDC notes only. On Paseo today that is a handful. The denomination ladder splits each crowd again, by rung. Both are the right long-term structure and both buy close to nothing at current volume. |
