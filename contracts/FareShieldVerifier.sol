...
/// @title FareShieldVerifier
/// @notice Groth16 verifier for the shield-note circuit
///         (circuits/shieldnote.circom), using the BN254 precompiles that
...
///
///         Same precompile path as FareLocationVerifier (already proven live on
///         Paseo Hub), specialized to this circuit's four public signals.
///
...
///
///         Public signals (4), truncated mod SCALAR_ORDER before pairing —
///         they are already field elements, this is defence in depth:
///           pub0 = root          — a recent vault note-tree root
///           pub1 = nullifierHash — Poseidon(nullifier), the spent-set key
///           pub2 = bucket        — denomination, bound into the leaf
///           pub3 = ksCommitment  — the shielded-pool commitment this deposit
///                                  must fund; binding it is what stops a
///                                  submitter re-pointing a valid proof
///
///         The verification key is set once by the owner after the trusted
///         setup (`node scripts/setup-shieldnote.mjs`). While unset,
///         verifyShieldNote returns false — fail-safe, so a mis-wired deploy
///         never deposits against a zero VK.
contract FareShieldVerifier is Ownable2Step {
    // -------------------------------------------------------------------------
    // BN254 constants
    // -------------------------------------------------------------------------
    ...
    uint256 private constant NUM_PUBLIC_INPUTS = 4;
    ...
    struct VerifyingKey {
        ...
        uint256[2] IC1; // root
        uint256[2] IC2; // nullifierHash
        uint256[2] IC3; // bucket
        uint256[2] IC4; // ksCommitment
    }
    ...
    ///         5 IC points (IC0..IC4) for 4 public signals + the constant.
    ...
    function setVerifyingKey(
        ...
        bytes32 vkHash = keccak256(
            abi.encode(alpha1, beta2, gamma2, delta2, IC0, IC1, IC2, IC3, IC4)
        );
    ...
    /// @notice Verify a shield-note spend. Returns false (never reverts) on a
    ///         missing VK or a malformed proof, so callers treat "not proven"
